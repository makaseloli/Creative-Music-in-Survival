...
public final class DimensionMusicSoundInstance extends SimpleSoundInstance {
    private DimensionMusicSoundInstance(Identifier location) {
    ...
    public static SimpleSoundInstance forMusic(DimensionMusicSource choice, SoundEvent fallback) {
        List<Identifier> candidates = Minecraft.getInstance().getSoundManager().getAvailableSounds().stream()
                .filter(location -> matches(choice, location.getPath()))
                .toList();
        return candidates.isEmpty() ? SimpleSoundInstance.forMusic(fallback)
