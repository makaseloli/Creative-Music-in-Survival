...
public final class MusicReplacer {
    ...
    public static Music replace(Music original) {
        ...
        boolean biomeAvailable = original != null && isBiomeOrEnvironmentMusic(original);
        ...
        Music timingSource = original != null ? original : target == MusicGenre.CREATIVE ? Musics.CREATIVE : Musics.GAME;
        if (replacement == null) return null;
        return replacement.source() == DimensionMusicSource.OVERWORLD ? toMusic(timingSource, replacement.choice()) : timingSource;
    }
    ...
    private static Music toMusic(Music original, MusicChoice choice) {
        ...
        return new Music(selected.sound(), original.minDelay(), original.maxDelay(), original.replaceCurrentMusic());
    }
}
